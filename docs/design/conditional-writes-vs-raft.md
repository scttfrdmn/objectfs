...
- `PutObject` has **one production implementation** (`internal/storage/s3/backend.go:808`) and **five
  test doubles** (`tests/posix_test.go:443`, `tests/fuse_test.go:61`,
  `tests/predictive_cache_test.go:54`, `tests/integration_test.go:517`,
  `pkg/types/interfaces_test.go:32`). Changing the signature edits all six and every one of the nine
  non-test call sites, most of which have no precondition to express. A new method leaves them alone.
