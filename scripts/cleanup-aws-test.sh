...
if [[ -f .env.test ]]; then
    # .env.test is generated by scripts/setup-aws-test.sh and is gitignored, so shellcheck cannot
    # follow it (SC1091). There is nothing to follow at lint time by design — the file holds a bucket
    # name and a region for a machine this script is being run on.
