...
//
// The directory is 0750 and the file 0640 — owner writes, group reads, world nothing. They were 0755
// and 0644, which is world-readable, and what ObjectFS writes to a log is not public: object keys are
// the paths of a user's files, and every error string carries the bucket, the key and often the
// full request context. On the multi-user systems this filesystem is built for, a mount's log under
// 0644 hands every account on the host a listing of what the mount owner has been reading. #211 was
// the same mistake on the health endpoint, which reported component names and error strings on the
// wildcard address by default.
//
// Group-readable rather than 0600: an operator running ObjectFS under a service account with an
// operations group needs to read these without sudo, and that is the case the umask cannot express
// once O_CREATE has already picked a mode.
func (lr *LogRotator) openFile() error {
	// Create directory if it doesn't exist
	dir := filepath.Dir(lr.config.Filename)
	if err := os.MkdirAll(dir, 0o750); err != nil { // #nosec G301 -- 0750 is the fix; gosec wants 0750 or less and this is it
		return fmt.Errorf("failed to create log directory: %w", err)
	}

	// Open or create the log file
	file, err := os.OpenFile(lr.config.Filename, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o640) // #nosec G302 -- group-readable on purpose; see above
	if err != nil {
...
//
// filename is always a name this type generated — rotate passes backupFilename's output, which is
// filepath.Join(dir, prefix-timestamp+ext) over the operator-configured Filename. So the two G304
// findings here are the same finding as anywhere a configured path is opened: the path comes from the
// config file, and honoring it is what this type is for. There is no caller that can route an
// untrusted string to it, and it is unexported.
//
// The destination is 0640 via OpenFile rather than os.Create's 0666: a compressed backup holds exactly
// the log content openFile above declines to make world-readable, and rotating a log should not widen
// its permissions. gosec does not flag os.Create at all, which is why this one had to be reasoned
// about rather than reported.
func (lr *LogRotator) compressFile(filename string) error {
	// Open source file
	src, err := os.Open(filename) // #nosec G304 -- a name generated by backupFilename from configured Filename
	if err != nil {
		return err
	}
	...
	dst, err := os.OpenFile(filename+".gz", os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0o640) // #nosec G302,G304 -- see above: deliberate 0640, generated name
	if err != nil {
		return err
	}
